...
class lang_cond_MAE_add_multimodal_mask(nn.Module):
    """ Masked Autoencoder with VisionTransformer backbone, with language as conditioned input
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
                 embed_dim=1024, depth=24, num_heads=16,
...
class lang_gen_MAE(nn.Module):
    """ Masked Autoencoder with VisionTransformer backbone
    """
    def __init__(self, mae_weight=1.0, lm_weight=0.5, vocab_size=0, max_lang_len=40, eps = 1e-8,
        ...
        self.decoder_norm = norm_layer(decoder_embed_dim)
        self.decoder_pred = nn.Linear(decoder_embed_dim, patch_size**2 * in_chans, bias=True) # decoder to patch
        self.lang_prediction =  nn.Linear(decoder_embed_dim, vocab_size, bias=True)
        # --------------------------------------------------------------------------
        ...
        ## Attention!!! 在只是用一帧，也就是图片没有上下文信息的时候可以不需要这个ctx_xx_pe
        ...
        n_patches, total_seq = num_patches, num_patches + max_lang_len + 1 # +1 for cls token
        prefix_mask = torch.tril(torch.ones((total_seq, total_seq), dtype=torch.uint8))
        prefix_mask[:n_patches+1, :n_patches+1] = 1 # +1 for cls token
    ...
    def forward_decoder(self, x, ids_restore, lang_gen, lang_gen_mask):
        # embed tokens
        x = self.decoder_embed(x) # [N, 49+1, decoder_embed_dim]
        projected_lang_gen = self.lang2decoder(lang_gen)

        # append mask tokens to sequence
        mask_tokens = self.mask_token.repeat(x.shape[0], ids_restore.shape[1] + 1 - x.shape[1], 1) #[N, 196-49, decoder_embed_dim]
        x_ = torch.cat([x[:, 1:, :], mask_tokens], dim=1)  # no cls token
        x_ = torch.gather(x_, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, x.shape[2]))  # unshuffle
        x = torch.cat([x[:, :1, :], x_], dim=1)  # append cls token
        # add pos embed
        ...
        decoder_patches_mask = torch.ones_like(x[..., -1], dtype=lang_gen_mask.dtype)
        ...
        patches = multimodal_embedding[:, : -lang_gen_mask.shape[-1], ...]
...
def lang_gen_MAE_test_patch16_dec512d8b(**kwargs):
    model = lang_gen_MAE(
        vocab_size=30522,
        max_lang_len=40,
...
lgen_MAE_base_patch16 = lang_gen_MAE_base_patch16_dec512d8b
lgen_MAE_large_patch16 = lang_gen_MAE_large_patch16_dec512d8b
lgen_MAE_test_patch16 = lang_gen_MAE_test_patch16_dec512d8b
